<?php
// $Id: funkce.inc.php 112 2007-08-29 08:20:39Z Mira $
/**
 * @file funkce.php
 * Soubor obsahuje vöechny funkce 
 */
include_once('mysqli.inc.php');
include_once('connect_info.inc.php');
...
function mysql_query_e($dotaz,$link){
  ...
  or die("$echo <br> 
  Nelze vykonat definicn√≠ dotaz: <br> 
  $dotaz <br> 
...
function formln($popis,$telo,$title=null){
  echo "<tr><td title=\"$title\" align=\"right\">$popis</td><td align=left>$telo</td></tr>\n";
}

function input_form($label,$name,$value, $size = null){
  static $i = 0;
  $i++;
  $lab = "<label for=\"labelid$i\">$label</label>";
  if ($size != null){
    formln($lab,"<input id=\"labelid$i\" name=\"$name\" size = $size value=\"$value\">");
  }
  else {
    formln($lab,"<input id=\"labelid$i\" name=\"$name\" value=\"$value\">");
  }
}
function psswd_form($label,$name,$value,$size=null){
  static $i =0;
  $i++;
  $str = "<input id=\"labelpswdid$i\" type=password name=\"$name\" ".(($size != null)?"size=$size":"").
  ((strlen ($value) != 0)?"value=$value":"").">";
  $lab = "<label for=\"labelpswdid$i\">$label</label>";
  formln($lab,$str);
//  if ($size != null){
...
function submit_form($value,$name="submit",$size=null){
  if ($size != null){
    formln("","<input class=\"tlacitko\" type=\"submit\" name=$name size=$size value =\"$label\">");
    
  }
  else{
    formln("","<input class=\"tlacitko\" type=\"submit\" name=$name  value =\"$value\">");
  }

}
function submit($value,$name="submit",$size=null){
  if ($size != null){
    echo "<input class=\"tlacitko\" type=\"submit\" name=$name size=$size value =\"$label\">";
    
  }
  else{
    echo "<input class=\"tlacitko\" type=\"submit\" name=$name  value =\"$value\">";
  }
}
...
//funkce ovƒõ≈ôuje zda jsou √∫daje z√°kazn√≠ka v po≈ô√°dku
function check_zakaznik($zakaznik){
  if ((strlen(trim($zakaznik["jmeno"])) < 3)){
    $out[] = "Jm√©no mus√≠ b√Ωt alespo≈à t≈ôi znaky dlouh√©!";
  }
  if ((strlen($zakaznik["email"])>0)&(!jeEmail($zakaznik["email"]))){
    $out[] = "Nespr√°vn√Ω tvar emailu!";
  }
  return $out;
}
...
// vr√°t√≠ pole voln√Ωch z√°kazn√≠k≈Ø v≈°etnƒõ doporuƒçen√Ωch
...
//  Zakaznik_mereni left join Mereni // zobrazÌ i nulovÈ z·znamy
  //var_dump($vysledek);
  //echo "<br>";
  ...
  if ($zaznamy != null){
    foreach($zaznamy as $key => $zaznam){
      //var_dump($zaznamy);
      //vybere t≈ôi nejpodobn≈°j≈°√≠ z√°kazn√≠ky
      ...
      "order by Pocet desc limit 1;",$link);
...
/// @returns VracÌ poËet zak·zek v archivu.
function pocet_seznamu_v_archivu($zakid,$link){
  $sql_result = mysql_query_e(
  "select count(*) as Pocet from Seznam 
  where (IDZakaznik = ".$zakid.") and (Deleted <> true) and (Archiv = true);",$link);
  $pocet=mysqli_fetch_assoc($sql_result);
  return $pocet["Pocet"];
}


function pocet_zakazniku_v_archivu($link){
  $sql_result = mysql_query_e(
  "select count(*) as Pocet from Zakaznik 
  where (Deleted <> true) and (Archiv = true);",$link);
  $pocet=mysqli_fetch_assoc($sql_result);
  return $pocet["Pocet"];
}
...
function otevrene_seznamy($idzak,$link){
  ...
  order by IDSeznam 
  ;",$link);
  //or die("Nelze vykonat definicn√≠ dotaz: " . mysql_error());
 ...
 */
function zarad_mereni_do_seznamu($id_seznam,$id_zak_mer,$link){
  /*$OtevreneSeznamy = mysql_query_e(
  "select IDSeznam from Seznam 
  where (Deleted <> true)and(Archiv <> true)and(IDZakaznik =".$id_zak.") 
  ...
  set IDSeznam = ".$id_seznam." 
  where IDZakaznik_mereni = ".$id_zak_mer.";",$link)   //TODO
  or die("Nelze vykonat definicn√≠ dotaz: " . mysql_error());
  //echo "huh";
}
/**
 * Funkce nastavÌ ceny v obecenÈm cenÌku
 * Ceny smrku a borovice jsou nastveny z "Cena_1", kter· p¯edstavuje mÏkkÈ
 * a ceny dubu a buku jsou z "Cena_4", kter· p¯edstavuje tvrdÈ d¯ÌvÌ.  
 */
function setCeny($ceny,$link){
  mysql_query_e("update Druh_dreva set Cena =".$ceny["Cena_1"]."where IDDruh_dreva = 1",$link);
  mysql_query_e("update Druh_dreva set Cena =".$ceny["Cena_1"]."where IDDruh_dreva = 2",$link);
  mysql_query_e("update Druh_dreva set Cena =".$ceny["Cena_1"]."where IDDruh_dreva = 3",$link);
  mysql_query_e("update Druh_dreva set Cena =".$ceny["Cena_4"]."where IDDruh_dreva = 4",$link);
  mysql_query_e("update Druh_dreva set Cena =".$ceny["Cena_4"]."where IDDruh_dreva = 5",$link);
...
// Funkce vr√°t√≠ ceny z obecn√©ho cen√≠ku.
function getCeny($link){
  ...
  $zaznam = mysqli_fetch_assoc($vysledek); // je≈°tƒõ nen√≠ implementovan√°
...
//funkce zkop√≠ruje ceny z aktu√°ln√≠ho cen√≠ku do cen√≠ku seznamu.
...
// funkce vytvo≈ô√≠ nov√Ω seznam, nakop√≠ruje do nƒõj honoty cen, z obecn√©ho cen√≠ku
// a vr√°t√≠ jeho id.
...
// Funkkce kontroluje zda existuje v datab√°zi u≈æivatel s t√≠mto nickem a s 
//t√≠mto heslem. 
...
/**
 * Funkce vypoËÌt· cenu za kubick˝ metr danÈho stromu.
 */
function cena_strom_m3($mereni, $ceny){
  ...
  $out = $CenaZaDruh + 
  ($mereni["IDManipulace"] == 1?1:0)*$ceny["Cena_man"] +
  ($mereni["IDDvakrat"] == 1?1:0)*$ceny["Cena_dvakrat"] +
  $CenaZaDalsi;
...
function cena_strom($mereni,$ceny){
  $cena = cena_strom_m3($mereni,$ceny);

  $out = $mereni["Objem"]*$cena;
  
  return $out;
}


//Vr√°t√≠ seznam z√°kazn√≠k≈Ø
function zakaznici($link,$archiv = false){
  ...
  if (mysqli_num_rows($vysledek)== 0){
    return 0;
    //echo "Nenalezen ≈æ√°dn√Ω odpov√≠daj√≠c√≠ ≈ô√°dek!";
  }
  while ($zaznam=MySQLi_Fetch_assoc($vysledek)){
    $out[] = $zaznam;
  }
...
//funkce sma≈æe z√°kazn√≠ka
// TODO 
// Je t≈ôeba dodƒõlat, aby ≈°el vymazat jen pokud m√° v≈°echny seznamy v uzav≈ôen√Ω a 
// nen√≠ na nƒõj ≈æ√°dnej volnej strom. 
...
function new_mereni_do_seznamu($idSeznam,$mereni,$link){
  ...
  mysql_query_e("
    insert into 
    Mereni
    (IDSeznam,Delka,Prumer,IDDruh_dreva,IDDvakrat,IDManipulace,IDDalsi_zpracovani,Status)" 
    ." values(".
    "$idSeznam,".
    $mereni["Delka"].",".
    $mereni["Prumer"].",".
    ...
    $mereni["IDDvakrat"].",".
    $mereni["IDManipulace"].",".
    $mereni["IDDalsi_zpracovani"].", \"normal\");",$link);
...
function seznam_mereni($idseznam,$link,$serad = false){
  $razeni = $serad?"(Mereni.IDDruh_dreva > 3), IDManipulace, IDDvakrat, IDDalsi_zpracovani,Mereni.IDDruh_dreva ":"Mereni.IDMereni";
  $vysledek = mysql_query_e(
...
/**
 * Funkce obnovÌ ceny v urËitÈm seznamu. Ceny jsou jen dvÏ za tvrdÈ a mÏkkÈ 
 * d¯ÌvÌ. V p¯edchozÌ implementaci vöak byly ceny za kaûd˝ druh d¯eva zvl·öù.
 * Funkce byla jen poupravena a v datab·zi jsou st·le informace o cen·ch u 
 * kaûdÈho druhu zvl·öù. Jen se berou z jednoho okÈnka formul·¯e.  
 */
...
function update_mereni($zaznamy,$link){
  foreach($zaznamy as $key => $zaznam){
    if (empty($zaznam["IDDalsi_zpracovani"])){
      $zaznam["IDDalsi_zpracovani"] = "NULL";
    }
...
function sql_datum($sql_date,$day=true){
  ...
  $dny = array("Ne","Po","√öt","St","ƒåt","P√°","So");
